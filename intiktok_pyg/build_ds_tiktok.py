...
def get_artist_map() -> dict:
    ...
    return get_x_map("./intiktok_pyg/ds/artist_map.pkl")


def get_track_map() -> dict:
    logging.info("Loading track dict...")
    return get_x_map("./intiktok_pyg/ds/track_map.pkl")


def get_tag_map() -> dict:
    logging.info("Loading tag dict...")
    return get_x_map("./intiktok_pyg/ds/tag_map.pkl")
...
def build_artist_map():
    ...
    with driver.session() as session:
        query = "MATCH (n:Artist {in_tiktok: true}) RETURN n.main_id AS main_id;"
        ...
        with open("./intiktok_pyg/ds/artist_map.pkl", "wb") as out_file:
...
def build_track_map():
    ...
    result_map = dict()
    with driver.session() as session:
        query = "MATCH (t:Track)<-[:WORKED_IN]-(a:Artist {in_tiktok: true}) RETURN DISTINCT t.id AS id;"
        q_result = session.run(query)
        result_map = {
            record["id"]: i
            for i, record in enumerate(q_result)
        ...
        with open("./intiktok_pyg/ds/track_map.pkl", "wb") as out_file:
            pickle.dump(result_map, out_file)
    driver.close()
...
def build_tag_map():
    ...
    with driver.session() as session:
        query = """
            MATCH (a:Artist {in_tiktok: true})<-[:TAGS]-(t:Tag)
            RETURN t.id AS id
            UNION
            MATCH (:Artist {in_tiktok: true})-[:WORKED_IN]->(:Track)<-[:TAGS]-(t:Tag)
            RETURN t.id AS id
        """
        q_result = session.run(query)
        result_map = {
        ...
        with open("./intiktok_pyg/ds/tag_map.pkl", "wb") as out_file:
            pickle.dump(result_map, out_file)
    driver.close()
...
def build_artist_tensor():
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/artists.pt")
...
def build_track_tensor():
    ...
    with driver.session() as session:
        query = """
            MATCH (n:Track)<-[:WORKED_IN]-(:Artist {in_tiktok: true})
            WITH DISTINCT n
            WITH
                n.id as id,
                n.popularity_scaled AS popularity_scaled,
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/tracks.pt")
...
def build_tag_tensor():
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/tags.pt")
...
def build_worked_in_by_tensor():
    ...
    driver = GraphDatabase.driver(f"bolt://{DB_HOST}:{DB_PORT}", auth=basic_auth(DB_USER, DB_PASS))  # type: ignore
    count = get_x_count("(:Artist {in_tiktok: true})-[:WORKED_IN]->(:Track)", driver)
    logging.info("Got %d records of WORKED_IN", count)
    result_tensor = torch.empty((2, count), dtype=torch.long)
    with driver.session() as session:
        query = """
            MATCH (n:Artist {in_tiktok: true})-[:WORKED_IN]->(m:Track)
            RETURN
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/worked_in.pt")
    logging.info("Saving worked_by tensor...")
    result_tensor[[0, 1], :] = result_tensor[[1, 0], :]
    torch.save(result_tensor, "./intiktok_pyg/ds/worked_by.pt")
...
def build_collab_with_tensor():
    ...
    driver = GraphDatabase.driver(f"bolt://{DB_HOST}:{DB_PORT}", auth=basic_auth(DB_USER, DB_PASS))  # type: ignore
    count = get_x_count("(:Artist {in_tiktok: true})-[:COLLAB_WITH]->(:Artist {in_tiktok: true})", driver)
    logging.info("Got %d records of COLLAB_WITH", count)
    result_tensor = torch.empty((2, count), dtype=torch.long)
    attr_tensor = torch.empty((count, 1), dtype=torch.float32)
    with driver.session() as session:
        query = """
            MATCH (n:Artist {in_tiktok: true})-[r:COLLAB_WITH]->(m:Artist {in_tiktok: true})
            WHERE n < m
            RETURN
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/collab_with.pt")
    logging.info("Saving collab_with attr tensor...")
    torch.save(attr_tensor, "./intiktok_pyg/ds/collab_with_attr.pt")
...
def build_musically_related_to_tensor():
    ...
    driver = GraphDatabase.driver(f"bolt://{DB_HOST}:{DB_PORT}", auth=basic_auth(DB_USER, DB_PASS))  # type: ignore
    count = get_x_count("(:Artist {in_tiktok: true})-[:MUSICALLY_RELATED_TO]->(:Artist {in_tiktok: true})", driver)
    logging.info("Got %d records of MUSICALLY_RELATED_TO", count)
    result_tensor = torch.empty((2, count), dtype=torch.long)
    attr_tensor = torch.empty((count, 1), dtype=torch.float32)
    with driver.session() as session:
        query = """
            MATCH (n:Artist {in_tiktok: true})-[r:MUSICALLY_RELATED_TO]->(m:Artist {in_tiktok: true})
            WHERE n < m
            RETURN
                n.main_id AS artist0_id,
                m.main_id AS artist1_id,
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/musically_related_to.pt")
    logging.info("Saving musically_related_to attr tensor...")
    torch.save(attr_tensor, "./intiktok_pyg/ds/musically_related_to_attr.pt")
...
def build_personally_related_to_tensor():
    ...
    driver = GraphDatabase.driver(f"bolt://{DB_HOST}:{DB_PORT}", auth=basic_auth(DB_USER, DB_PASS))  # type: ignore
    count = get_x_count("(:Artist {in_tiktok: true})-[:PERSONALLY_RELATED_TO]->(:Artist {in_tiktok: true})", driver)
    logging.info("Got %d records of PERSONALLY_RELATED_TO", count)
    result_tensor = torch.empty((2, count), dtype=torch.long)
    attr_tensor = torch.empty((count, 1), dtype=torch.float32)
    with driver.session() as session:
        query = """
            MATCH (n:Artist {in_tiktok: true})-[r:PERSONALLY_RELATED_TO]->(m:Artist {in_tiktok: true})
            WHERE n < m
            RETURN
                n.main_id AS artist0_id,
                m.main_id AS artist1_id,
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/personally_related_to.pt")
    logging.info("Saving personally_related_to attr tensor...")
    torch.save(attr_tensor, "./intiktok_pyg/ds/personally_related_to_attr.pt")
...
def build_linked_to_tensor():
    ...
    driver = GraphDatabase.driver(f"bolt://{DB_HOST}:{DB_PORT}", auth=basic_auth(DB_USER, DB_PASS))  # type: ignore
    count = get_x_count("(:Artist {in_tiktok: true})-[:LINKED_TO]->(:Artist {in_tiktok: true})", driver)
    logging.info("Got %d records of LINKED_TO", count)
    result_tensor = torch.empty((2, count), dtype=torch.long)
    attr_tensor = torch.empty((count, 1), dtype=torch.float32)
    with driver.session() as session:
        query = """
            MATCH (n:Artist {in_tiktok: true})-[r:LINKED_TO]->(m:Artist {in_tiktok: true})
            WHERE n < m
            RETURN
                n.main_id AS artist0_id,
                m.main_id AS artist1_id,
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/linked_to.pt")
    logging.info("Saving linked_to attr tensor...")
    torch.save(attr_tensor, "./intiktok_pyg/ds/linked_to_attr.pt")
...
def build_last_fm_match_tensor():
    ...
    driver = GraphDatabase.driver(f"bolt://{DB_HOST}:{DB_PORT}", auth=basic_auth(DB_USER, DB_PASS))  # type: ignore
    count = get_x_count("(:Artist {in_tiktok: true})-[:LAST_FM_MATCH]->(:Artist {in_tiktok: true})", driver)
    logging.info("Got %d records of LAST_FM_MATCH", count)
    result_tensor = torch.empty((2, count), dtype=torch.long)
    attr_tensor = torch.empty((count, 1), dtype=torch.float32)
    with driver.session() as session:
        query = """
            MATCH (n:Artist {in_tiktok: true})-[r:LAST_FM_MATCH]->(m:Artist {in_tiktok: true})
            WHERE n < m
            RETURN
                n.main_id AS artist0_id,
                m.main_id AS artist1_id,
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/last_fm_match.pt")
    logging.info("Saving last_fm_match attr tensor...")
    torch.save(attr_tensor, "./intiktok_pyg/ds/last_fm_match_attr.pt")
...
def build_follows_tensor():
    ...
    driver = GraphDatabase.driver(f"bolt://{DB_HOST}:{DB_PORT}", auth=basic_auth(DB_USER, DB_PASS))  # type: ignore
    count = get_x_count("(:Artist {in_tiktok: true})-[:FOLLOWS]->(:Artist {in_tiktok: true})", driver)
    logging.info("Got %d records of FOLLOWS", count)
    result_tensor = torch.empty((2, count), dtype=torch.long)
    attr_tensor = torch.empty((count, 1), dtype=torch.float32)
    with driver.session() as session:
        query = """
            MATCH (n:Artist {in_tiktok: true})-[r:FOLLOWS]->(m:Artist {in_tiktok: true})
            RETURN
                n.main_id AS artist0_id,
                m.main_id AS artist1_id,
                r.weight as weight
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/follows.pt")
    logging.info("Saving follows attr tensor...")
    torch.save(attr_tensor, "./intiktok_pyg/ds/follows_attr.pt")
...
def build_tags_has_tag_tensor_artists():
    ...
    driver = GraphDatabase.driver(f"bolt://{DB_HOST}:{DB_PORT}", auth=basic_auth(DB_USER, DB_PASS))  # type: ignore
    count = get_x_count("()-[:TAGS]->(:Artist {in_tiktok: true})", driver)
    logging.info("Got %d records of TAGS", count)
    result_tensor = torch.empty((2, count), dtype=torch.long)
    with driver.session() as session:
        query = """
            MATCH (n:Tag)-[:TAGS]->(m:Artist {in_tiktok: true})
            RETURN
                n.id AS tag_id,
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/tags_artists.pt")
    logging.info("Saving has_tag tensor for artists...")
    result_tensor[[0, 1], :] = result_tensor[[1, 0], :]
    torch.save(result_tensor, "./intiktok_pyg/ds/has_tag_artists.pt")
...
def build_tags_has_tag_tensor_tracks():
    ...
    with driver.session() as session:
        count_query = """
            MATCH (:Artist {in_tiktok: true})-[:WORKED_IN]->(m:Track)
            WITH DISTINCT m
            MATCH (n:Tag)-[:TAGS]->(m)
            RETURN count(*) as c
        """
    ...
    torch.save(result_tensor, "./intiktok_pyg/ds/tags_tracks.pt")
    logging.info("Saving has_tag tensor for tracks...")
    result_tensor[[0, 1], :] = result_tensor[[1, 0], :]
    torch.save(result_tensor, "./intiktok_pyg/ds/has_tag_tracks.pt")
...
if __name__ == '__main__':
    ...
    if not os.path.exists("./intiktok_pyg/ds/"):
        os.mkdir("./intiktok_pyg/ds/")
