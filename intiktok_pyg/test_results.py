...
def test(model, loader, criterion, device, train_edges_set, model_name, best_threshold, inv_artist_map):
    model.eval()
    all_labels = []
    all_probs = []
    true_positives = []
    ...
    with torch.no_grad(), open(f"{model_name}_timings.data", "wb") as out_file:
        for batch in tqdm.tqdm(loader, desc="Testing"):
            ...
            for src, dst, label in zip(edge_label_index[0, :], edge_label_index[1, :], edge_label):
                ...
                # Balancing logic from original script
            ...
            # RESULTS
            for i in range(filtered_edges_tensor.size(1)):
                label = filtered_labels_tensor[i].item()
                prob = probs[i].item()

                # Check if it's a True Positive
                if prob > best_threshold and label == 1:
                    src_local_idx = filtered_edges_tensor[0][i].item()
                    dst_local_idx = filtered_edges_tensor[1][i].item()
                    ...
                    true_positives.append([main_id0, main_id1])
    ...
    return test_loss / valid_batches if valid_batches > 0 else 0, all_labels, all_probs, true_positives
...
def find_best_threshold_from_csv(model_name, year, perc, filepath="intiktok_pyg/results.csv"):
    try:
        with open(filepath, mode="r") as file:
            reader = csv.DictReader(file)
            for row in reader:
                # Check for matching model, year, perc and if training is marked as done
                if (row["model"] == model_name and
                        int(row["year"]) == year and
                        float(row["perc"]) == perc and
                        row["done"].strip().lower() == "true"):
                    print(f"Found matching 'done' entry in results.csv.")
                    return float(row["best_threshold"])
    except FileNotFoundError:
        print(f"Warning: Results file not found at {filepath}")
    except Exception as e:
        print(f"Warning: Could not read threshold from CSV: {e}")
    return None


def main():
    parser = argparse.ArgumentParser(description="GNN Testing for TikTok Heterodata")
    ...
    model_name = "nocat_intiktok"
    test_hd_filename = f"full_hd_{perc}_intiktok.pt"

    # We also need the training edges to filter them out during testing
    train_collab_with_filename = f"collab_with_{year}_{month}_{perc}_intiktok.pt"

    # Model path
    model_path = f"intiktok_pyg/trained_models/model_{model_name}_{year}_{month}_{perc}.pth"
    ...
    best_threshold = find_best_threshold_from_csv(model_name, year, perc)
    if best_threshold is not None:
        print(f"Found best threshold from results.csv: {best_threshold}")
    else:
        print(f"Could not find threshold in results.csv. Using default: {best_threshold}")
        exit(1)
    ...
    print("Starting Test...")
    test_loss, all_labels, all_probs, true_positives = test(model, test_loader, criterion, device, train_edges_set, model_name, best_threshold, inv_artist_map)

    # WRITE RESULTS TO CSV
    csv_filename = f"./intiktok_pyg/results_{year}_{month}_{perc}.csv"
    with open(csv_filename, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["artist_a", "artist_b"])
        writer.writerows(true_positives[::2])
