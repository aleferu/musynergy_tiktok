...
def main(driver: Driver, tags: list[str], filename: str) -> None:
    ...
    with driver.session() as session:
        for t0, t1 in combinations_with_replacement(tags, 2):
            ...
            query = f"""
                MATCH (n:Artist {{main_tag: "{t0}", in_tiktok: true}})-[r:FOLLOWS]->(m:Artist {{main_tag: "{t1}", in_tiktok: true}})
                WHERE n < m
                WITH COUNT(r) AS c
...
if __name__ == '__main__':
    ...
    main(driver, good_tags, "good_follows_tags")

    # main(driver, bad_tags, "bad_follows_tags")

    # main(driver, good_tags + bad_tags, "all_follows_tags")
